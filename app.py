...
if st.button("Predict Stress"):
    ...
    if sleep < 6: st.write("🛌 Maintain a proper sleep schedule.")
    if study > 7: st.write("📚 Take regular breaks during study time.")
    if social > 5: st.write("📱 Limit your social media usage.")
    if pressure > 3: st.write("🎯 Manage academic tasks step by step.")
    if family < 3: st.write("👨‍👩‍👧 Seek support from family or close ones.")
    if activity < 2: st.write("🏃 Include physical activity in your routine.")
    if screen > 7: st.write("💻 Reduce screen time and take breaks.")
    if financial > 3: st.write("💰 Discuss financial concerns with a trusted person.")
    if examfear > 3: st.write("📝 Practice regularly to build confidence.")
